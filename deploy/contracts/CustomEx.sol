...
contract CustomDex {
    ...
    string[] public tokens = ["ETH", "MON", "Tether USD", "USD Coin"];
    ...
    // --- Safety checks added (require) to provide clearer revert reasons ---
    function getBalance(string memory tokenName, address _address) public view returns (uint256) {
        require(address(tokenInstanceMap[tokenName]) != address(0), "Token not found");
    ...
    function swapEthToToken(string memory tokenName) public payable returns (uint256) {
        ...
        // Ensure dex contract has enough token balance to send
        require(tokenInstanceMap[tokenName].balanceOf(address(this)) >= outputValue, "Dex out of tokens");
    ...
    function swapTokenToEth(string memory tokenName, uint256 _amount) public returns (uint256) {
        require(address(tokenInstanceMap[tokenName]) != address(0), "Token not found");
        // convert the amount (ethValue) to exact amount (integer division)
        ...
        // First take tokens from user, then transfer ETH — prevents reentrancy/exploit
        require(tokenInstanceMap[tokenName].transferFrom(msg.sender, address(this), _amount), "Token transferFrom failed");

        // send ETH to the user
        (bool sent, ) = payable(msg.sender).call{value: ethToBeTransferred}("");
    ...
    function swapTokenToToken(string memory srcTokenName, string memory destTokenName, uint256 _amount) public {
        require(address(tokenInstanceMap[srcTokenName]) != address(0), "Source token not found");
        require(address(tokenInstanceMap[destTokenName]) != address(0), "Destination token not found");

        require(tokenInstanceMap[srcTokenName].transferFrom(msg.sender, address(this), _amount), "Source transferFrom failed");
        require(tokenInstanceMap[destTokenName].transfer(msg.sender, _amount), "Destination transfer failed");
    ...
    // Allow contract to receive ETH (required to have ETH balance)
...
//     function swapTokenToEth(string memory tokenName, uint256 _amount) public returns (uint256) {
//         // convert the amount (ethValue) to exact amount 10
//         uint256 exactAmount = _amount / 10 ** 18;
//         uint256 ethToBeTransferred = exactAmount * ethValue;
//     require(address(this).balance >= ethToBeTransferred, "Dex is running low on balance");
